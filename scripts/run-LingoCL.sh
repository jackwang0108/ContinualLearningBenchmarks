...
# passing user-defined templates for generating text embeddings
cd "$root_dir" && python -m main --model "${cl_algo}" --backbone "resnet34" --dataset "cifar100" --num_tasks 10 --fixed_tasks --lr 0.1 --epochs 160 --batch_size 32 --each_buffer_size 20 --templates "a image of [CLS]" --log_times 10 --gpu_id 0 --name "NAME OF THE LOGGING DIR" --message "LEAVE YOUR COMMENT OF THIS RUNNING" 
